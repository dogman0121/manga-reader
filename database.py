...
class Comment:
    @staticmethod
    def add(title_id, text, user_id, parent=None):
        cursor.execute(
            """ INSERT INTO comments(title_id, parent, user_id, text, date) VALUES(?, ?, ?, ?, ?) """,
                       (title_id, parent, user_id, text, datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],)
        )
        connection.commit()
