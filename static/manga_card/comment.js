class Comment{
         ...
         let elementHTML = `
            <div class="comment">
                <div class="comment__user">
                    <img class="user__avatar" src="images/user-avatar.jpg">
    ...
    sendAnswer(event) {
        let answerFormInput = this.comment.answerForm.querySelector(".comments__input");
        fetch("../add_comment", {
            method: "POST",
            headers: {
                'Accept': 'application/json',
            ...
            .then((comment) => {
                ...
                if (this.comment.mainParent){
                    ...
                    if (this.comment.answers.length === 0){
                        ...
                        if (!this.comment.element.querySelector(".answers__list"))
                            this.comment.showAnswers();
...
class TitleComment extends Comment {
    ...
    loadAnswers(commentId) {
        return fetch("../get_answers?"+ new URLSearchParams({comment_id: commentId}))
            .then(response => response.json());
    }
}
